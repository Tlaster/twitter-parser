...
internal class LegacyParserEngine(
    ...
    fun parse(input: String): List<Token> {
        ...
        while (index < input.length) {
            ...
            var kind: LegacyTokenKind? = null
            val end = when (input[index]) {
                'h' -> scanUrl(input, index).also { if (it > index) kind = LegacyTokenKind.Url }
                '@', '＠' -> scanUserName(input, index).also { if (it > index) kind = LegacyTokenKind.UserName }
                '#', '＃' -> scanHashTag(input, index).also { if (it > index) kind = LegacyTokenKind.HashTag }
                '$', '＄' -> scanCashTag(input, index).also { if (it > index) kind = LegacyTokenKind.CashTag }
                ':' -> scanEmoji(input, index).also { if (it > index) kind = LegacyTokenKind.Emoji }
                else -> -1
            }
            if (kind != null) {
